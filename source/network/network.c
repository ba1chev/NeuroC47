...
forward_cache_t* network_forward(const network_t* network, const vector_t* X) {
    ...
    for (size_t i = 0; i < network->count_of_layers; i++) {
        layer_forward(network->layers[i], cache->activations[i], cache, i + 1);
    }
...
void network_backward(network_t* network, const forward_cache_t* cache, const vector_t* y, float learning_rate) {
    ...
    for (size_t layer_index = count_of_layers; layer_index-- > 0;) {
        ...
        if (layer_index > 0) {
            ...
            if (layer->activation_derivative && cache->pre_activations[layer_index]) {
                vector_t* deriv = layer->activation_derivative(cache->pre_activations[layer_index]);
                avx_256_vector_hadamard(prev_delta, deriv, prev_delta);
                free_vector(deriv);
                free(deriv);
            }
