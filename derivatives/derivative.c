...
vector_t* calculate_derivative_at(const derivative_t* derivative_obj, const vector_t* X) {
    ...
    for (size_t i = 0; i < X->size; i++) {
        ...
        vector_t* left_value = derivative_obj->function(left_part);
        vector_t* right_value = derivative_obj->function(right_part);
        result->data[i] = (left_value->data[i] - right_value->data[i]) / (2.0f * derivative_obj->precision);

        free_vector(left_value); free(left_value);
        free_vector(right_value); free(right_value);
